...
names(gene_sets) <- c("LAP3_STATE_UNION", "LAP3_STATE_UNION_NO_TRANSLATION_PROTEOSTASIS")
gene_sets <- lapply(gene_sets, function(x) sort(setdiff(x, "LAP3")))
target_gene_names <- sort(unique(c("LAP3", unlist(gene_sets, use.names = FALSE))))
