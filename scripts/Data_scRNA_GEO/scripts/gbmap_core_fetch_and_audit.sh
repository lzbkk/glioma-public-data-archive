...
status_file="${logs_dir}/gbmap_core_fetch.status"
wrapper_log="${logs_dir}/gbmap_core_fetch_wrapper.log"
heartbeat_file="${logs_dir}/gbmap_core_fetch.heartbeat.log"
target_file="${data_dir}/core_gbmap_cellxgene.h5ad"
source_url="https://datasets.cellxgene.cziscience.com/861acfd8-25f0-418b-a445-aa96da232827.h5ad"
expected_size="8127057404"
...
printf '%s | START GBmap Core fetch and entry audit\n' "$(date '+%F %T')"
printf '%s | source=%s\n' "$(date '+%F %T')" "${source_url}"
...
Rscript --vanilla "${project_dir}/Data_scRNA_GEO/scripts/gbmap_core_h5ad_entry_audit.R" \
  "${target_file}" "${results_dir}"

printf '%s  %s\n' "$(sha256sum "${target_file}" | cut -d' ' -f1)" "$(basename "${target_file}")" \
  > "${results_dir}/core_gbmap_cellxgene.sha256"

printf '%s | DONE GBmap Core fetch and entry audit\n' "$(date '+%F %T')"
