...
message("Reading H5AD structure without loading expression matrix")
h5_index <- h5ls(input_file, recursive = TRUE, all = TRUE)
write.csv(
  h5_index,
  file.path(output_dir, "core_gbmap_h5_structure.csv"),
  row.names = FALSE
)
...
write.csv(
  obs_index,
  file.path(output_dir, "core_gbmap_obs_structure.csv"),
  row.names = FALSE
)
...
write.csv(
  field_table,
  file.path(output_dir, "core_gbmap_obs_fields.csv"),
  row.names = FALSE
)
...
write.csv(
  required_checks,
  file.path(output_dir, "core_gbmap_entry_requirements.csv"),
  row.names = FALSE
)

summary_lines <- c(
  "# Core GBmap H5AD Entry Audit",
  "",
  ...
  "",
  "This structure audit does not yet prove that every GSE131928-derived cell can be",
  "excluded. The next step must decode candidate categorical columns and quantify",
  "cells and donors by source before any biological analysis."
)
writeLines(summary_lines, file.path(output_dir, "README.md"))

message("H5AD structure audit complete")
