...
Status: repository route fixed; GitHub repository created; Zenodo DOI/record URL pending

## 2026-07-02 Public Archive Decision

```text
Decision: GitHub repository + Zenodo archived release DOI.
Decision record: Project_Management/Decisions/Public_Archive_DOI_Decision_20260702.md.
Freeze audit: Project_Management/Audits/20260702/Public_Archive_Freeze_Audit_20260702.md.
Zenodo metadata draft: Project_Management/Submission_Package/ZENODO_METADATA_DRAFT_20260702.md.
Upload checklist: Project_Management/Submission_Package/PUBLIC_ARCHIVE_UPLOAD_CHECKLIST_20260702.md.
```

The generated source-data package, journal-facing supplementary tables and analysis code should be archived through this route before submission. The 2026-07-02 freeze audit verified the archive file set, regenerated the source-data/supplementary packages after the NOA reference final pass, and fixed upload/do-not-upload boundaries. The public GitHub repository has been created at `https://github.com/lzbkk/glioma-public-data-archive`; the final Zenodo DOI/record URL, release version and licence remain pending author-side confirmation and upload.
...
The final manuscript submission should include the public GitHub repository at https://github.com/lzbkk/glioma-public-data-archive and a versioned Zenodo record for the generated source-data package, journal-facing supplementary tables and analysis code. The Zenodo DOI, Zenodo record URL, release version and final licence are pending. Until these fields are created and inserted, the generated source-data package and code availability statement should be treated as not submission-frozen.
```
...
Analysis scripts used to generate the reported tables, source data and figures will be archived before submission in a public GitHub repository (https://github.com/lzbkk/glioma-public-data-archive) linked to a versioned Zenodo DOI [Zenodo DOI pending]. The archived code release should include the R scripts for TCGA/CGGA bulk analyses, LAP3-state construction, alternative-anchor benchmarking, submodule decomposition, Core GBmap projection, GBM-Space topology analysis, CPTAC/GLASS projection, figure generation and source-data indexing, together with a README describing expected inputs, access-controlled inputs that must be obtained from third-party repositories, software versions and the order of execution. The code release will not include controlled-access raw data or third-party datasets that the authors are not permitted to redistribute.
```
...
| Generated Figure 1-5 source data | Direct support for manuscript figures | Within paper/supplement plus public archive recommended | 30 source-data rows in `supplementary_source_data_index_20260701.csv`; DOI pending | Deposit with manuscript Source Data and final repository |
| Supplementary Tables 1-11 | Gene sets, projection tables, software, provenance and mechanism landscape audit | Within paper/supplement plus public archive recommended | `supplementary_table_manifest_20260701.csv`; DOI pending | Deposit as Supplementary Tables and final repository |
| Analysis scripts | Reproducibility | Public code repository plus Zenodo release DOI recommended | GitHub repository `https://github.com/lzbkk/glioma-public-data-archive`; Zenodo DOI pending | Archive release before submission; exclude credentials and controlled raw data |
...
|---|---|---|
| Persistent identifier for generated artifacts | GitHub repository created; Zenodo DOI missing | Populate repository and create Zenodo DOI before submission |
...
| Licence | Missing | Choose licence for generated code/data; do not apply open licence to third-party raw data |
...
1. **Blocking before submission:** generated source-data/code archive DOI is not yet created.
2. **Resolved as of 2026-07-03:** GitHub repository URL is fixed as `https://github.com/lzbkk/glioma-public-data-archive`.
3. **Important:** final licence for generated source data and code is recommended but not author-confirmed.
...
6. 归档路线已固定为 `GitHub + Zenodo DOI`。GitHub repository 已创建：`https://github.com/lzbkk/glioma-public-data-archive`。下一步是确认 licence、上传 Zenodo record，并把 DOI/record URL/release version 回填到本文档和最终稿。
